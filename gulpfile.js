...
gulp.task('ship-to-gitlab', function (done) {
    ...
    var gitPath = 'https://' + user + ':' + token + `@gitlab.syncfusion.com/content/wpf-docs`;
    console.log('Clone has been started...!');
    var clone = shelljs.exec('git clone ' + gitPath + ' -b ' + branch + ' ' + `../../../gitlabRepo/wpf-docs`, {
        silent: false
    });
    if (clone.code !== 0) {
        ...
        for (var changedFileName of changedFileNames.split(',')) {
            ...
            if (changedFileName !== null && changedFileName !== '' && changedFileName !== '.gitignore' && !changedFileName.includes('.gitlab')) {
                ...
                if (fs.existsSync('../wpf-docs/' + changedFileName)) {
                    // It will update the modified files
                    if (fs.existsSync('../../../gitlabRepo/wpf-docs/' + changedFileName)) {
                        shelljs.cp('-rf', `../wpf-docs/` + changedFileName, `../../../gitlabRepo/wpf-docs/` + changedFileName);
                    }
                    else {
                        // It will update the newly added files
                        if (fs.existsSync('../../../gitlabRepo/wpf-docs/')) {
                            shelljs.cp('-rf', `../wpf-docs/` + changedFileName, `../../../gitlabRepo/wpf-docs/` + changedFileName);
                        }
                    }
                    ...
                    if (fs.existsSync('../../../gitlabRepo/wpf-docs/' + changedFileName)) {
                        shelljs.rm('-rf', `../../../gitlabRepo/wpf-docs/` + changedFileName);
        ...
        shelljs.cd(`../../../gitlabRepo/wpf-docs`);
